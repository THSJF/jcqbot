...
public class DicReply extends BaseModule{
	...
	public BaseModule load() {
		...
		try {
			if (ConfigManager.instence.SanaeConfig.dicRegex.get(-1) == null) {
				regexMode = false;
			} else {
				regexMode = ConfigManager.instence.SanaeConfig.dicRegex.get(-1);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void setRegexMode(long fromGroup, boolean isReg) {
		ConfigManager.instence.SanaeConfig.dicRegex.put(fromGroup, isReg);
	...
	public boolean processMsg(long fromGroup, long fromQQ, String msg, int msgId) {
		...
		DicReplyGroup drg=groupMap.get(fromGroup);
		if (drg == null) {
			drg = new DicReplyGroup(fromGroup);
			groupMap.put(fromGroup, drg);
		}
		return drg.checkMsg(fromGroup, fromQQ, msg);
	}
	...
	private class DicReplyGroup {
		...
		public DicReplyGroup(long group) {
			...
			dic = Autoreply.gson.fromJson(Tools.FileTool.readString(dicFile), type);
			if (ConfigManager.instence.SanaeConfig.dicRegex.get(groupNum) == null) {
				regexMode = false;
			} else {
				regexMode = ConfigManager.instence.SanaeConfig.dicRegex.get(groupNum);
			}
		}
