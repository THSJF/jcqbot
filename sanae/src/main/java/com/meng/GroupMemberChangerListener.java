...
public class GroupMemberChangerListener {
    ...
    public void checkIncrease(int subtype, int sendTime, long fromGroup, long fromQQ, long beingOperateQQ) {
        PersonInfo personInfo = ConfigManager.instence.getPersonInfoFromQQ(beingOperateQQ);
        if (personInfo != null && personInfo.name.equals("熊哥")) {
            Autoreply.sendMessage(Autoreply.mainGroup, 0, "熊加入了群" + fromGroup);
            return;
        }
        if (personInfo != null) {
            Autoreply.sendMessage(fromGroup, 0, "欢迎" + ConfigManager.instence.getNickName(beingOperateQQ) + "来到本群");
        } else {
            Autoreply.sendMessage(fromGroup, 0, "欢迎新人");
        }
		String wel=ConfigManager.instence.getWelcome(fromGroup);
		if (wel != null) {
    ...
    public void checkDecrease(int subtype, int sendTime, final long fromGroup, final long fromQQ, long beingOperateQQ) {
        if (subtype == 1) {
            ...
            Autoreply.sendMessage(fromGroup, 0, ConfigManager.instence.getNickName(beingOperateQQ)  + "(" + qInfo.getQqId() + ")" + "去寻找更适合" + (qInfo.getGender() == 1 ?"她": "他") + "的地方了");
        } else if (subtype == 2) {
            if (beingOperateQQ == Autoreply.CQ.getLoginQQ()) {
                ConfigManager.instence.addBlack(fromGroup, fromQQ);
                return;
            }
            QQInfo qInfo = Autoreply.CQ.getStrangerInfo(beingOperateQQ);
            QQInfo qInfo2 = Autoreply.CQ.getStrangerInfo(fromQQ);
			Autoreply.sendMessage(fromGroup, 0, ConfigManager.instence.getNickName(beingOperateQQ) + "(" + qInfo.getQqId() + ")" + "被" + ConfigManager.instence.getNickName(fromQQ) + "(" + qInfo2.getQqId() + ")" + "踢了一脚");
        }
    }
