...
public class RitsukageDataPack {
	...
	/*
	 ...
	 小律影向八云蓝发送查询类指令时，八云蓝返回的数据包中的时间戳会设置成和收到的数据包中时间戳相同
	 其实并不检查时间戳和实际时间,只是个任务标记

	 小律影→正邪  小律影发送 八云蓝接收
	 正邪→小律影  八云蓝发送 小律影接收 (其实是广播方式发送,所有连接的客户端都可以收到)
	 小律影↔正邪  都可以发送和接收
	...
	//小律影→正邪  获得八云蓝".jrrp"中的计算结果  n1:qq号
