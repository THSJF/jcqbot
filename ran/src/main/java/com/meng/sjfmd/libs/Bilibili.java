...
public class Bilibili {
	...
	public static VideoUrl getVideoUrl(long aid, long cid, int quality) {
		return GSON.fromJson(Network.httpGet("https://api.bilibili.com/x/player/playurl?avid=" + aid + "&cid=" + cid + "&qn=" + quality + "&type=flv", Autoreply.instance.cookieManager.getHina(), "https://www.bilibili.com/video/av" + aid), VideoUrl.class);
	}
	...
	public static UserInfo getUserInfo(long id) {
		return GSON.fromJson(Network.httpGet("https://api.bilibili.com/x/space/acc/info?mid=" + id + "&jsonp=jsonp", Autoreply.instance.cookieManager.getHina()), UserInfo.class);
	}
	...
	public static void sendFavorite(long uid, long aid, String cookie) {
		...
		try {
			...
			System.out.println(result.optInt("code") == 0 ?"收藏成功": bilibiliMainPost);
...
//			} catch (IOException e) {
//				Autoreply.instance.showToast("连接出错");
//				return;
//			}
//			if (response.statusCode() != 200) {
//				Autoreply.instance.showToast(String.valueOf(response.statusCode()));
//			}
//			JsonParser parser = new JsonParser();
//			JsonObject obj = parser.parse(response.body()).getAsJsonObject();
//			Autoreply.instance.showToast(obj.get("message").getAsString());
	}
