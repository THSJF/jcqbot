 ...
 * 具体功能可以查看文档
 */
public class Una extends JcqAppAbstract implements ICQVer, IMsg, IRequest {
	...
	public static void main(String[] args) {
		...
		ins.startup();// 程序运行开始 调用应用初始化方法
		ins.enable();// 程序初始化完成后，启用应用，让应用正常工作
		// ......
	...
	public String appInfo() {
		...
		String AppID = "com.airo.una";// 记住编译后的文件和json也要使用appid做文件名
		/**
	...
	//<editor-fold desc="Main Functions Define">
	...
	//<editor-fold desc="Other Functions Define">
		...
		public static File resetData() throws IOException {
			...
			for (Group group:groups) {
				List<Member> currentMembers=CQ.getGroupMemberList(group.getId());
				for (Member member:currentMembers) {
					Member cp=currentMembers.get(new Random().nextInt(currentMembers.size()));
					int luck=new Random().nextInt(10);
					result.add(new long[]{group.getId(),member.getQqId(),cp.getQqId(),luck});
				}
			}
			pw.write("");
		...
		public static LinkedList<long[]> loadData() throws IOException {
			...
			while (br.ready()){
				...
				long[] values={
						Long.valueOf(strValues[0]),
						Long.valueOf(strValues[1]),
						Long.valueOf(strValues[2]),
						Long.valueOf(strValues[3]),
				};
				result.add(values);
			}
			br.close();
